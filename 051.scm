...
(define (execute-move stacks from to)
  (let ((old-from (list-ref stacks (- from 1))))
    (let loop ((acc '())
      ...
      (cond ((null? stacks)
             (reverse acc))
            ((= i from)
             (loop (cons (cdr (car stacks))
                         acc)
                   (cdr stacks)
                   (+ i 1)))
            ((= i to)
             (loop (cons (cons (car old-from)
                               (car stacks))
                         acc)
                   (cdr stacks)
                   (+ i 1)))
...
(define (execute-action stacks action)
  (let loop ((count (car action))
             (stacks stacks))
    (if (= 0 count)
      stacks
      (loop (- count 1)
            (execute-move stacks (cadr action) (caddr action))))))
