...
(define (move-sand sc blocked-coords)
  (cond
    ((not (set-contains? blocked-coords (down sc)))
     (down sc))
    ((not (set-contains? blocked-coords (down (left sc))))
     (down (left sc)))
    ((not (set-contains? blocked-coords (down (right sc))))
     (down (right sc)))
    (else #f)))
...
(define (add-sand bmc blocked-coords)
  (let loop ((sand-coord (coord 500 0)))
    (let ((next-coord (move-sand sand-coord blocked-coords)))
      (if next-coord
        (if (> (coord-y next-coord) bmc)
          #f
          (loop next-coord))
        sand-coord))))
