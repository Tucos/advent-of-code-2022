...
class Node:
    ...
    def move(self, n):
        ...
        tgt = self
        # 0 means nothing, n means nothing, n-1 means nothing?
        # 0 1 2 3 4 5    ; move 0 5 right
        #            0
        #          0     ; move 0 1 left
        for i in range(self.x%n if self.x>=0 else n - (abs(self.x)%n)):
            tgt = step(tgt)
        if tgt is self:
            return
...
def list_to_string(nodes):
    head = next(filter(lambda n: n.x==0, nodes))
    lst = [head.x]
    node = head.next
...
def list_from_file(filename):
    with open(filename, 'rt') as f:
        ...
        nodes = [Node(int(x) * 811589153) for x in f.readlines()]
...
if __name__ == "__main__":
    ...
    #print(f'lst: {list_to_string(nodes)}')
    for i in range(10):
        for node in nodes: node.move(len(nodes)-1)
        #print(f'lst: {list_to_string(nodes)}')
